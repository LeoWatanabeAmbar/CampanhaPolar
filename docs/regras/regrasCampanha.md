...
- **Comprovação manual:** `lais.vendrasco@ambar.tech`, `leonardo.watanabe@ambar.tech`, `jorge.castro@ambar.tech`, `anyelle.santos@ambar.tech` e `luis.oliveira@ambar.tech` podem preencher e salvar os checks por região/mês; os demais usuários podem visualizar. Considerações operacionais impedem confirmar as fases somente a partir das vendas.
