...
**Implementação atualizada em 16/09/2026:** o painel disponível em [app.py](../app.py) contém visão geral, análise individual por região, Venda no Quadrimestre, clientes novos, clientes reativados, mix de produtos e preenchimento manual de adiantamento. Todos os usuários autenticados consultam; `lais.vendrasco@ambar.tech`, `leonardo.watanabe@ambar.tech`, `jorge.castro@ambar.tech`, `anyelle.santos@ambar.tech` e `luis.oliveira@ambar.tech` alteram o adiantamento. O acesso usa Supabase Auth e Data API, sem senha PostgreSQL no Streamlit. Instruções no [README do projeto](../README.md).
